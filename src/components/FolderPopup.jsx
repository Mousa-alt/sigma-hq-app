...
export default function FolderPopup({ project, folder, title, gcsFolderName: passedGcsFolderName, onClose }) {
  ...
  const handleItemClick = (item) => {
    if (item.type === 'folder') {
      ...
      setPathHistory([...pathHistory, { path: relativePath, title: item.name }]);
  ...
  const handleOpenExternal = (file) => {
    const viewUrl = file.path ? `${SYNC_WORKER_URL}/view?path=${encodeURIComponent(file.path)}` : file.url;
  ...
  const getDocBadge = (filename, path) => {
    ...
    const colorClasses = {
      ...
      sky: 'bg-sky-100 text-sky-700', slate: 'bg-slate-100 text-slate-600',
    };
    return <span className={`px-1.5 py-0.5 rounded text-[8px] font-semibold uppercase ${colorClasses[typeInfo.color] || colorClasses.slate}`}>{typeInfo.label}</span>;
  };
        ...
        <div className="bg-white rounded-2xl shadow-2xl w-full max-w-3xl max-h-[80vh] overflow-hidden flex flex-col" onClick={e => e.stopPropagation()}>
          ...
          <div className="p-5 border-b border-slate-100 flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div className="p-2 bg-slate-100 rounded-lg"><Icon name="folder-open" size={18} className="text-slate-600" /></div>
              <div>
            ...
            <div className="px-5 py-2 bg-slate-50 border-b border-slate-100 flex items-center gap-1 text-xs overflow-x-auto">
              {pathHistory.map((item, i) => (
                <span key={i} className="flex items-center gap-1 whitespace-nowrap">
                  ...
                  <button onClick={() => { setPathHistory(pathHistory.slice(0, i + 1)); setCurrentPath(item.path); }}
              ...
              {loading ? (
                ...
                <div className="divide-y divide-slate-100">
                  {files.map((item, i) => {
                    ...
                    return (
                      <div key={i} onClick={() => handleItemClick(item)} onDoubleClick={() => handleDoubleClick(item)}
                        ...
                        <div className="flex-1 min-w-0">
                          {!isFolder && <div className="flex items-center gap-2 mb-0.5">{getDocBadge(item.name, item.path)}{parsed?.revision && <span className="text-[9px] text-slate-400">Rev {parsed.revision}</span>}</div>}
                          <p className="text-sm font-medium text-slate-900 truncate">{isFolder ? item.name : (parsed?.description || item.name)}</p>
                          <p className="text-[10px] text-slate-400">{isFolder ? 'Double-click to open' : `${formatDate(item.updated)}${item.size ? ` • ${formatSize(item.size)}` : ''}`}</p>
                        </div>
              ...
              <div className="w-1/2 p-4 bg-slate-50 flex flex-col">
                ...
                <div className="bg-white rounded-lg border border-slate-200 p-4 mb-4 flex-1 overflow-y-auto">
                  {(() => {
                    ...
                    const typeInfo = getDocTypeInfo(detectDocumentType(selectedFile.name, selectedFile.path || ''));
                    return (
                      <div className="space-y-3">
                        <div><p className="text-[10px] text-slate-400 uppercase">Type</p><p className="text-sm font-medium text-slate-900">{typeInfo.description}</p></div>
                        {parsed.revision && <div><p className="text-[10px] text-slate-400 uppercase">Revision</p><p className="text-sm text-slate-900">Rev {parsed.revision}</p></div>}
                        {selectedFile.size && <div><p className="text-[10px] text-slate-400 uppercase">Size</p><p className="text-sm text-slate-900">{formatSize(selectedFile.size)}</p></div>}
                        {selectedFile.updated && <div><p className="text-[10px] text-slate-400 uppercase">Modified</p><p className="text-sm text-slate-900">{formatDate(selectedFile.updated)}</p></div>}
                      </div>
                    );
                  })()}
                </div>
                <div className="space-y-2">
                  <button onClick={() => setViewingFile(selectedFile)} className="w-full px-4 py-3 bg-slate-900 text-white rounded-lg text-sm font-medium hover:bg-slate-800 transition-colors flex items-center justify-center gap-2">
                    <Icon name="eye" size={16} />View File
                  </button>
                  <button onClick={() => handleOpenExternal(selectedFile)} className="w-full px-4 py-3 bg-slate-100 text-slate-700 rounded-lg text-sm font-medium hover:bg-slate-200 transition-colors flex items-center justify-center gap-2">
                    <Icon name="external-link" size={16} />Open in New Tab
                  </button>
                </div>
              </div>
            )}
          ...
          <div className="p-4 border-t border-slate-100 flex items-center justify-between bg-slate-50">
            <span className="text-xs text-slate-500">{files.filter(f => f.type === 'file').length} files{files.filter(f => f.type === 'folder').length > 0 && `, ${files.filter(f => f.type === 'folder').length} folders`}</span>
            <button onClick={() => project?.driveLink && window.open(project.driveLink, '_blank')} className="flex items-center gap-2 px-3 py-1.5 bg-white border border-slate-200 rounded-lg text-xs font-medium text-slate-700 hover:bg-slate-50 transition-colors">
              <Icon name="external-link" size={12} />Open in Drive
            </button>
          </div>
        </div>
      </div>
