...
export default function ProjectSettings({ project, onUpdateProject, onDeleteProject }) {
  ...
  return (
    <div className="max-w-2xl">
      <div className="mb-6">
        <h2 className="text-lg font-semibold text-slate-900 mb-1">Project Settings</h2>
        <p className="text-sm text-slate-500">Manage project details and configuration</p>
      </div>
      ...
      <div className="bg-white border border-slate-200 rounded-xl p-6 space-y-5">
        ...
        <div>
          <label className="block text-xs font-medium text-slate-700 mb-2">Google Drive URL</label>
          <input
            type="url"
        ...
        <div className="grid grid-cols-3 gap-4">
          ...
          <div>
            <label className="block text-xs font-medium text-slate-700 mb-2">Completion Date</label>
            <input
              type="date"
        ...
        <div className="flex items-center justify-between pt-4 border-t border-slate-100">
          <button
            onClick={handleSave}
            disabled={saving}
            className="px-6 py-3 bg-slate-900 hover:bg-slate-800 text-white rounded-xl text-sm font-medium transition-colors flex items-center gap-2 disabled:opacity-50"
          >
            {saving ? <Icon name="loader-2" size={16} className="animate-spin" /> : saved ? <Icon name="check" size={16} /> : <Icon name="save" size={16} />}
